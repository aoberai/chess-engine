...
with strategy.scope():
    ...
    input_position_conv = tf.keras.layers.Conv2D(
        16, (7, 7), padding="same", activation='relu')(input_position)
    input_position_conv2 = tf.keras.layers.Conv2D(
        16, (7, 7), padding="same", activation='relu')(input_position_conv)
    input_position_conv3 = tf.keras.layers.Conv2D(
        16, (5, 5), padding="same", activation='relu')(input_position_conv2)
    input_position_conv4 = tf.keras.layers.Conv2D(
        16, (5, 5), padding="same", activation='relu')(input_position_conv3)
    input_position_conv5 = tf.keras.layers.Conv2D(
        32, (3, 3), strides=2, activation='relu')(input_position_conv4)
    input_position_conv6 = tf.keras.layers.Conv2D(
        32, (3, 3), padding="same", activation='relu')(input_position_conv5)
    input_position_conv7 = tf.keras.layers.Conv2D(
        64, (3, 3), padding="same", activation='relu')(input_position_conv6)
    input_position_conv8 = tf.keras.layers.Conv2D(
        128, (3, 3), padding="same", activation='relu')(input_position_conv7)
    input_position_conv9 = tf.keras.layers.Conv2D(
        128, (2, 2), padding="same", activation='relu')(input_position_conv8)
    input_position_conv_final = tf.keras.layers.Conv2D(
        128, (2, 2), activation='relu')(input_position_conv9)
    ...
    output_dense = tf.keras.layers.Dense(
        units=output_nodes,
        activation='tanh')(flattened_input_position)
...
total_dataset = tf.data.Dataset.from_generator(
    generator=training_data_generator, output_signature=(
        tf.TensorSpec(
            shape=input_shape, dtype=np.int32), tf.TensorSpec(
                shape=(), dtype=np.float32)))
total_dataset.cache()

# val_dataset = tf.data.Dataset.from_generator(
# generator=validation_data_generator,
# output_signature=(tf.TensorSpec(shape=input_shape, dtype=np.int32),
# tf.TensorSpec(shape=(), dtype=np.float32)))
...
batch_size = 256
val_dataset = total_dataset.take(validation_set_size).batch(
    batch_size).prefetch(buffer_size=tf.data.experimental.AUTOTUNE)
train_dataset = total_dataset.skip(validation_set_size).batch(
    batch_size).prefetch(buffer_size=tf.data.experimental.AUTOTUNE)


early_stopping_callback = tf.keras.callbacks.EarlyStopping(
    monitor='val_loss', restore_best_weights=True, patience=6)
