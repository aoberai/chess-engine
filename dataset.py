...
positions = np.zeros((wanted_dataset_size, 8, 8, 6))
...
with open("data/chessData.csv") as file1, open("data/random_evals.csv") as file2:
    ...
    for line in combined_raw_dataset:
        ...
        if 'w' == position_eval[0].split()[1]:
            normalized_evaluation = 1 if "#" in position_eval[1] else 2 * (1 / (1 + math.exp(-float(
                position_eval[1]) / 300)) - 0.5)  # normalizes centipawn score with sigmoid function
            # position_eval[0] = position_eval[0] + (input_length -
            # len(position_eval[0])) * " " # Add padding on FEN
            position_array = np.asarray(
                [i.split(" ") for i in str(chess.Board(position_eval[0])).split("\n")])
